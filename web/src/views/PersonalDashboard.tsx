...
import { Copy, Gauge, Info, KeyRound, Layers, LogOut, MonitorSmartphone, Plus, RefreshCw, Route, Save, UserRound } from "lucide-react";
...
import type { AccountUsage, PersonalPayload, WorkspaceMembershipView } from "../types";
import type { DashboardData } from "../hooks/useDashboardData";

// Personal dashboard has two distinct non-admin modes:
//   - user: website session; manages workspaces and device tokens
//   - client: device/PAT bearer; reads its own usage, dispatches, and token setup
export function PersonalDashboard({
  ...
  const { t } = useLang();
  const isUserMode = personal.mode === "user";
  const title = isUserMode ? personal.user?.username || t("用户", "User") : personal.client?.label || personal.client?.id || "client";
  ...
  return (
      ...
      navbar={
        <div className="cube-navbar sticky top-0 z-20 flex min-h-14 w-full items-center justify-between gap-3 border-b border-slate-200 cube-glass px-4 py-2">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Button aria-label={isUserMode ? "Logout" : "Clear token"} className="gap-2" isDisabled={busy} size="sm" variant="danger-soft" onPress={onClearToken}>
              <LogOut size={15} />
              <span className="hidden sm:inline">{isUserMode ? t("退出", "Logout") : t("令牌", "Token")}</span>
            </Button>
          </div>
        </div>
      }
    >
      <div className="cube-content mx-auto flex w-full max-w-6xl flex-col gap-4 p-3 sm:p-4 lg:gap-5 lg:p-6">
        {isUserMode ? (
          <UserPersonalContent busy={busy} data={data} personal={personal} />
        ) : (
          <ClientPersonalContent
            busy={busy}
...
function UserPersonalContent({ busy, data, personal }: { busy: boolean; data: DashboardData; personal: PersonalPayload }) {
  const { t } = useLang();
  const user = personal.user;
  const workspaces = personal.workspaces || [];
  const activeDevices = (personal.devices || data.devices).filter((device) => device.active).length;

  return (
    <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <MetricCard icon={<UserRound size={18} />} label={t("用户", "User")} value={user?.username || "-"} status={user?.disabled ? "danger" : "success"} />
        <MetricCard icon={<MonitorSmartphone size={18} />} label={t("活跃设备", "Active devices")} value={activeDevices.toString()} status={activeDevices ? "success" : "warning"} />
        <MetricCard icon={<Layers size={18} />} label={t("工作区", "Workspaces")} value={workspaces.length.toString()} status={workspaces.length ? "success" : "warning"} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
        <Card className="cube-card">
          ...
          <Card.Content className="gap-3 text-sm">
            <SignalLine label={t("用户名", "username")} value={user?.username || "-"} />
            <SignalLine label={t("用户 ID", "user id")} value={user?.id || "-"} />
            <SignalLine label={t("最近登录", "last login")} value={shortTime(user?.lastLoginAt)} />
            <SignalLine label={t("状态", "status")} value={user?.disabled ? t("已禁用", "disabled") : t("活跃", "active")} />
          </Card.Content>
        </Card>

        <UserWorkspacesCard busy={busy} data={data} workspaces={workspaces} />
      </div>
  ...
  return (
    <Card className="cube-card">
      <Card.Header className="flex items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
        ...
        <Chip color={workspaces.length ? "success" : "warning"} variant="soft">
          {workspaces.length}
        </Chip>
      </Card.Header>
      <Card.Content className="gap-4">
        <div className="divide-y divide-slate-100 rounded-lg border border-slate-200">
          {workspaces.map((workspace) => (
            <div key={workspace.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3">
              <div className="min-w-0">
                <div className="truncate text-sm font-semibold text-slate-950">{workspace.name || workspace.id}</div>
                <div className="font-mono text-[11px] text-slate-400">{workspace.id}</div>
              </div>
              <Chip color={workspace.role === "admin" ? "accent" : "default"} size="sm" variant="soft">
                {workspace.role}
              </Chip>
            </div>
          ))}
          {!workspaces.length && <div className="px-4 py-5 text-sm text-slate-500">{t("暂无工作区", "No workspaces")}</div>}
        </div>

        <div className="grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_auto]">
          <Input
            fullWidth
            aria-label={t("工作区名称", "workspace name")}
          ...
          <Button
            ...
            variant="primary"
            onPress={async () => {
              await data.createWorkspace(name.trim());
              setName("");
            }}
          >
            <Plus size={15} />
...
function ClientPersonalContent({
  busy,
  onApplyToken,
  onTokenInput,
  personal,
